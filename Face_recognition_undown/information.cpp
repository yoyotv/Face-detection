...
void class_information::calculate_all_average_face() {
	...
	for (int i = 0; i < number_of_face_class; i++) {
		for (int j = 0; j < figure_height*figure_width; j++) {
			class_average_face[i][j] = class_average_face[i][j] + all_image_matrix[i][j];
		}
...
void classify::determine_result() {
	if (choice == 1) {
		cout << "According to the Euclidian distance and Equation(8)" << endl;
		cout << "We can say this image is Class " << criteria_answer << " !" << endl;
	}
	else {
		for (int i = 0; i < choice; i++) {
			if (all_result[i] == test_data_ground_truth[i]) {
				percentage++;
			};
		}
		percentage = percentage / choice * 100;
		cout << "According to the Euclidian distance and Equation(8)" << endl;
		cout << "The accuracy of testing data is " << percentage << " % !" << endl;
	}
